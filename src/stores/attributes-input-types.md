...
|Page Builder|A fully-functioning [Page Builder]({{ site.baseurl }}{% link cms/page-builder.md %}) workspace at the attribute location that makes it easy to add engaging content to the product page.|<!--{% endif %}-->
|Date|Date values can be entered by making a selection from a drop-down list or popup calendar ( ![Calendar icon]({{ site.baseurl }}{% link images/images/btn-calendar.png %}){: .Inline} ). <br/><br/>**_Note:_** Depending on your system configuration, Admin users can type dates directly into a field or select a date from the calendar or list. For information about specifying date and time values, see [Date and Time Custom Options]({{ site.baseurl }}{% link stores/attribute-date-time-options.md %}).|
|Yes/No|Displays a drop-down list with pre-defined options of `Yes` and `No`.|
|Dropdown|Displays a drop-down list of values that accepts only a single selection. The Dropdown input type is a key component of [configurable products]({{ site.baseurl }}{% link catalog/product-create-configurable.md %}).|
...
|Media Image|Associates an additional image with a product, such as a product logo, care instructions, or ingredients from a food label. When you add a media image attribute to the attribute set of a product, it becomes an additional image type, along with Base, Small, and Thumbnail. The media image attribute can be excluded from the [media gallery]({{ site.baseurl }}{% link catalog/media-gallery.md %}).|
|Fixed Product Tax|Lets you define [FPT rates]({{ site.baseurl }}{% link tax/fixed-product-tax.md %}) based on the requirements of your locale.|
|Visual Swatch|Displays a swatch that depicts the color, texture, or pattern of a configurable product. A [visual swatch]({{ site.baseurl }}{% link catalog/swatches.md %}) can be filled with a hexadecimal color value, or display an uploaded image that represents the color, material, texture, or pattern of the option.|
|Text Swatch|A text-based representation of a configurable product option that is frequently used for size. [Text swatches]({{ site.baseurl }}{% link catalog/swatches.md %}) can also include hexadecimal color values.|
