...
1. Complete the **Customers Grid and Segment Properties**.
   ...
   - To filter the Customers grid by text attribute with different filter matching conditions, set **Grid Filter Condition Type** to `Partial Match`, `Prefix Match` and `Full Match`.
...
|Grid Filter Condition Type|Specifies the text attribute filter matching conditions for [search]({% link stores/admin-workspace.md %}) operations from the grid. Options: Partial Match / Prefix Match / Full Match|
