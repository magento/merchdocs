...
|Grid Filter Condition Type|Specifies the text attribute filter matching conditions for [search]({% link stores/admin-workspace.md %}) operations from the grid. Options: Partial Match / Prefix Match / Full Match. Doesn't affect **_Search by keyword_** search field on the grid|
