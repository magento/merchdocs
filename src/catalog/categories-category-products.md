...
{:.bs-callout-tip}
When setting up a category rule, the products are _sorted_, _matched_, _assigned_ and _unassigned_ according to this rule when this category is saved **only**. For example, if you add a new product to the catalog and want to assign it according to the rule, you must **re-save each category** that is set to match products by rule to ensure that the new product is assigned. And if any product stock status is changed to `In Stock` or `Out of Stock`, and products in category should be _sorted_ according to the **Automatic Sorting** rule, you must click <span class="btn">Save Category</span> as well.
