...
1. Upon completion of the task, look for the file in the download location for your web browser.
    ...
    {:.bs-callout-info}
    By default, all exported files are located in `<Magento root directory>/var/export` folder. If remote storage like Amazon S3 is enabled, all exported files are located in `<Remote storage root directory>/import_export/export` folder.
