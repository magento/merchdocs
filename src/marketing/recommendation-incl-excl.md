...
- **Out of Stock Status** - (Available as an exclusion only.) Excludes products that are out of stock.
- **Low in Stock Status** - (Available as an exclusion only.) Excludes products that are low in stock. Low stock status is based on the _Only X left Threshold_ value in [Inventory configuration]({% link configuration/catalog/inventory.md %}).
