...
The dynamic block is displayed only when the rule fires. If the promo is targeted to a segment A, the block is shown to segment A. If the promo is not active, the block is not displayed.
