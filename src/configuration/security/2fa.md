...
|--- |--- |--- |
|Providers to use|Global|Indicates the two-factor authentication solution(s) that can be used to log in to the _Admin_. Authentication by only one 2FA solution is required to log in. If more than one 2FA solution is implemented, each user can choose the one they prefer to use.<br />Before logging in to the _Admin_, each user must follow the instructions from the provider to install the app or complete other setup requirements. For complete details, see [Two-Factor Authentication]({% link stores/security-two-factor-authentication.md %}).|
