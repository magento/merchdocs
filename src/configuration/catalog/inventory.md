...
|Enable Inventory Check On Cart Load|Global|When set to `Yes`, inventory check on cart item load is performed. Disabling inventory check skips pre validation of cart item data. It can lead to performance improvements on major checkout steps especially when there are a lot of items in cart. But because inventory check is skipped, some out of stock scenarios may throw other types of errors. Options: Yes / No|
