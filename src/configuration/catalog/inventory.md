...
|Use deferred Stock update|Global| Determines whether to defer stock update if Backorders are allowed ((i.e) The Store > Configuration > Catalog > Inventory > Product Stock Options > Back orders can be set to anything besides the default value **"No backorders"**). It works for a single product or an entire website and uses the **Job Queue** mechanism to allow the inventory quantity indicators to update **asynchronously** after the orders are placed. It's an Enterprise Edition feature.|
